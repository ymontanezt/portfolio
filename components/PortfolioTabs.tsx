...
import { Award, Briefcase, Calendar, Code, Eye, Github } from "lucide-react";
                ...
                {experience?.map((exp, index) => (
                  <AccordionItem key={exp.id} value={`exp-${exp.id}`}>
                    <Card>
                      <AccordionTrigger
                        value={`exp-${exp.id}`}
                        className="px-6 py-4 hover:no-underline w-full"
                      >
              ...
              <div>
                ...
                {(() => {
                  ...
                  const categoryColors = {
                    ...
                    "Base de Datos":
                      "from-orange-500/10 to-yellow-500/5 border-orange-500/20",
                    "Cloud & DevOps":
                      "from-indigo-500/10 to-blue-500/5 border-indigo-500/20",
                    Librerías:
                  ...
                  return categoryOrder.map((category) => {
                    ...
                    const translatedCategory =
                      Object.keys(groupedSkills || {}).find(
                        (key) =>
                          categoryTranslations[
                            key as keyof typeof categoryTranslations
                          ] ===
                            categoryTranslations[
                    ...
                    const categorySkills =
                      groupedSkills?.[translatedCategory] || [];
                    if (categorySkills.length === 0) return null;

                    return (
                      <div key={category} className="mb-8">
                        <div
                          className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-semibold mb-4 bg-gradient-to-r ${categoryColors[category as keyof typeof categoryColors]} border`}
                        >
                          {categoryTranslations[
                            category as keyof typeof categoryTranslations
                          ] || category}
